...
There is no commerce-search endpoint or commerce API key. During a retail Hunt, Hermes launches its browser, opens one brand- or merchant-first Bing results page, clicks only result references observed in its snapshot, and uses an observed official catalog's own search controls. After a catalog snapshot exposes an exact product name, Apt's read-only `browser_observed_link` tool resolves that visible name to same-origin link destinations already present in the page DOM. Hermes navigates to the returned direct URL and inspects the product snapshot, then returns with `browser_back` and repeats; it must not invent or guess product URLs. Direct `browser_console` use is blocked. Product candidates with a merchant homepage, shared catalog URL, or duplicate canonical URL fail server validation. Grocery and food Hunts likewise use only observed public store or menu results. A cold browser command has 45 seconds to complete; if the initial navigation omits its snapshot, Hermes requests one snapshot before continuing. The Apt-managed `apt-hunt-browser-policy` plugin removes upstream Hermes' API-backed `web_search` fallback from the model surface, blocks repeated unchanged-page snapshots, and limits each Hunt to 20 serialized browser calls with no more than seven explicit navigations. Provisioning fails if the required browser primitives are absent. The agent then calls `apt_commerce_hunt` with at most five typed candidates. Apt validates the candidate schema, vertical, freshness, direct public credential-free source URLs, DNS destinations, and coarse-location requirement before recording the Hunt. The browser boundary forbids authentication, accounts, contact/payment entry, terms acceptance, carts, checkout, purchases, orders, reservations, scheduling, merchant contact, and tracking.
