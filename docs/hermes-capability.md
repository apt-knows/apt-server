...
- exact discovery of the eight allowlisted Apt MCP tools through Hermes’ MCP health command;
- only `memory`, `session_search`, `skills`, the required browser navigation/click/type primitives, and Hermes’ constrained dynamic discovery/call tools on the initial model surface; Apt's read-only observed-product-link resolver is registered in the browser toolset and available through that constrained discovery path, while the policy plugin removes the upstream browser bundle's `web_search` fallback and blocks direct browser-console use;
- an external headless-browser navigation, accessibility snapshot, link click, and resulting cross-site navigation, with Apt's 45-second cold-command allowance;
