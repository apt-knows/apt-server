...
import type { CommerceSearchAdapter } from './commerce.js';
import { COMMERCE_MAX_DURATION_MS } from './commerce.js';
import {
...
export class ClawService {
  ...
  constructor(
    ...
    private readonly commerce: CommerceSearchAdapter,
  ...
  async invoke(context: ClawRunContext, tool: ClawToolName, rawArguments: unknown) {
    ...
    const input = commerceHuntInputSchema.parse(rawArguments);
    ...
    const timeout = setTimeout(() => controller.abort(), COMMERCE_MAX_DURATION_MS);
    try {
      const candidates = await this.commerce.search(input, context.location, controller.signal);
